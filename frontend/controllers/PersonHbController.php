...
use frontend\models\PersonHbSearch;
 ...
 */
class PersonHbController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $searchModel = new PersonHbSearch();
    ...
    public function actionView($id)
    {
        ...
        $followupModel = $model = FollowupHb::find()
        ...
        $person_dup = PersonHb::find()->select('id')->where(['cid' => $personModel->cid, 'date_in' => $personModel->date_in])->count();
    ...
    public function actionDelete($id)
    {
        ...
        $personsqc = PersonHb::find()->select('id')->where(['cid' => $cid])->all();
        if ($person == NULL) {
            FollowupHb::deleteAll(['cid' => $cid]);
        }
    ...
    {
        if (($model = PersonHb::findOne($id)) !== null) {
            return $model;
        }
    ...
    {
        if (($model = FollowupHb::find()->where('cid > :cid', [':cid' => $id])->one()) !== null) {
            return $model;
        }
