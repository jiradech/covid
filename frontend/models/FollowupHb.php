 ...
 */
class FollowupHb extends \yii\db\ActiveRecord
{
    public $person_id;
    ...
    {
        if ($insert) {
            $person = PersonHb::findOne($this->cid);
            $this->cid = $person->cid;

        } else {
            $person = PersonHb::findOne($this->person_id);
        }
        ...
        if ($person != NULL) {
            ...
            $followup = FollowupHb::find()->select('MAX(report_date) AS report_date')->where(['cid' => $this->cid])->one();
    ...
    public function afterSave($insert, $changedAttributes) 
    {
        ...
        if ($this->temp >= 37.5 & $date == date("Y-m-d")) {
            ...
            $person = PersonHb::find()->where(['cid' => $this->cid])->one();
            if ($person) {
