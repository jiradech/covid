 ...
 */
class FollowupSqc extends \yii\db\ActiveRecord
{
    public $person_id;
    ...
    {
        if ($insert) {
            $person = PersonSqc::findOne($this->cid);
            $this->cid = $person->cid;

        } else {
            $person = PersonSqc::findOne($this->person_id);
        }
        ...
        if ($person != NULL) {
            ...
            $followup = FollowupSqc::find()->select('MAX(report_date) AS report_date')->where(['cid' => $this->cid])->one();
    ...
    public function afterSave($insert, $changedAttributes) 
    {
        ...
        if ($this->temp >= 37.5 & $date == date("Y-m-d")) {
            ...
            $person = PersonSqc::find()->where(['cid' => $this->cid])->one();
            if ($person) {
                ...
                $this->notify_message2('แจ้งการเฝ้าระวังพบบุคคลมีไข้'.$amphur.' ดูข้อมูลเพิ่มเติมได้ที่: http://www.sko.moph.go.th/covid19/index.php?r=person-sqc/view&id='.$person->id,
