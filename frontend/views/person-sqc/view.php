            ...
            <div class="countcorona">
                ...
                <div class="countcorona-area">
                    ...
                    <?php
                        if ($person_dup > 1) {
                    ?>
<a href="<?=Url::to(['person-sqc/index', 'PersonSearch[cid]' => $model->cid, 'PersonSearch[date_in]' => $model->date_in])?>"><h5 style="color: red">พบมีการบันทึกคนซ้ำในระบบ <?=$person_dup?> ราย กรุณาตรวจสอบ [คลิก]</h5></a>
                    ...
                    <?= DetailView::widget([
                        ...
                        'attributes' => [
                            ...
                            [
                                ...
                                'value' =>
                                'บ้านเลขที่ '.$model->move_number.' หมู่ '.
                                ArrayHelper::getValue($model->getVillageArray($model->move_tambon . $model->move_vill_no), $model->move_tambon . $model->move_vill_no, 'ไม่ระบุ').
                           ...
                           'objective',
                            ...
                            //'remark',
                             ...
                             [
                                'attribute' => 'สถานที่เคยถูกสั่งให้ แยกกัก กักกัน หรือ คุมไว้สังเกต',
                                'value' =>
                                'บ้านเลขที่ '.$model->quarantine_number.' หมู่ '.
                                ArrayHelper::getValue($model->getVillageArray($model->quarantine_tambon . $model->quarantine_vill_no), $model->quarantine_tambon . $model->quarantine_vill_no, 'ไม่ระบุ').
                                ' ตำบล' . ArrayHelper::getValue($model->getTambonArray($model->quarantine_tambon), $model->quarantine_tambon, 'ไม่ระบุ') . ' อำเภอ' . ArrayHelper::getValue($model->getAmphurArray($model->quarantine_ampur), $model->quarantine_ampur, 'ไม่ระบุ') . ' จังหวัด' . ArrayHelper::getValue($model->getProvinceArray($model->quarantine_province), $model->quarantine_province, 'ไม่ระบุ').' ( ตั้งแต่วันที่ '.
                                (($model->date_quarantine_s ==0) ? 'ไม่ระบุ': Yii::$app->thai->thaidate('d F Y', strtotime($model->date_quarantine_s))).' ถึงวันที '.(($model->date_quarantine_e ==0) ? ' ไม่ระบุ ': Yii::$app->thai->thaidate('d F Y', strtotime($model->date_quarantine_e))).')'

                            ],
         //                  [
         // 'attribute' => 'date_quarantine_s',
         // 'value' => (($model->date_quarantine_s ==0) ? 'ไม่ระบุ': Yii::$app->thai->thaidate('d F Y', strtotime($model->date_quarantine_s))),
         // ],
                        //    'date_quarantine_s',
                      //      'date_quarantine_e',
                            [
                   ...
                   <h4>ข้อมูลการติดตามเฝ้าระวัง 14 วัน</h4>
                    <p>
                        <?= Html::a('เพิ่มรายงานติดตามผู้เดินทาง 14 วัน', ['followup-sqc/create', 'cid' => $model->id, 'person_id' => $model->id], ['class' => 'btn btn-success']) ?>
                    </p>
