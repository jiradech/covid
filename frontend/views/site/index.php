...
<section class="corona-count-section padding-tb pt-0">
	<div class="container">
		<div class="corona-wrap">
			<div class="corona-count-top wow fadeInUp">
						...
						<div class="corona-item">
							<div class="corona-inner">
								...
								<div class="corona-content">
									<p>ติดเชื้อสะสม</p>
									<h5><span>จ.สระแก้ว</span> <?=number_format($cumulative_pos)?></h5>
				...
				<div class="row justify-content-center align-items-center migrate">
					<div class="col-sm-2">
						<h5>สถานะประชากรเคลื่อนย้าย :</h5>
						<a href="<?= Url::to(['person/create']) ?>" class="btn btn-primary" style="width: 100%; margin-bottom: 4px;">บันทึกข้อมูลบุคคลเคลื่อนย้าย</a>
						<a href="<?= Url::to(['person/index']) ?>" class="btn btn-success" style="width: 100%">ค้นหาบุคคล</a>
					</div>
					...
					<div class="col-sm-2">
						<div class="corona-item">
							<div class="corona-inner">
								...
								<div class="corona-content text-center corona-stat">
						...
						<div class="corona-item">
							<div class="corona-inner">
								...
								<div class="corona-content text-center corona-stat">
									...
									<h3><?= number_format($sumary[0]['total']-$sumary[0]['pass14days']) ?></h3>
					...
					<div class="col-sm-2">
						<div class="corona-item">
							<div class="corona-inner">
								<div class="corona-thumb" style="margin: 0 auto;">
									<img src="<?php echo $this->theme->baseUrl ?>/assets/images/corona/06.png" alt="corona">
								</div>
								<div class="corona-content text-center corona-stat">
									<a href="<?= Url::to(['observe']) ?>">
									<p>คุมไว้สังเกต</p>
									<h3><?= number_format($observe[0]['live']) ?></h3>
									</a>
								</div>
							</div>
						</div>
...
<!-- 					<div class="col-sm-2">
			...
			<div class="countcorona">
				<div class="countcorona-area" style="padding: 0px 16px;">
					<?= $this->render('/locate/_form_locate', [
						'model' => $model,
						'tambon' => [], //$tambon,
						'village' => [], //$village
					]) ?>
			...
			<div class="corona-count-bottom row">
						<div class="col-12">
							<h6>คลิกตำแหน่งบนแผนที่ เพื่อแสดงข้อมูลรายหมู่บ้าน</h6>
						</div>
				...
				<div>
					...
					<span class="badge badge-pui">สีแดง = มี PUI (รอผล/อยู่ระหว่าเฝ้าระวัง)</span>
			...
			<script>
				...
				?>
						...
						L.marker([<?= $value['coordinates'] ?>], {
								...
								sick_result: '<?= $value['person_in_risk'] ?>',
				...
				function onMapClick(e) {
					...
					$('#detected').removeClass("badge badge-active");
					if (e.target.options.detected > 0) {
						$('#detected').addClass("badge badge-active");
					}

					$('#sick_result').text(e.target.options.sick_result);
					$('#sick_result').removeClass("badge badge-active");
					if (e.target.options.sick_result > 0) {
						$('#sick_result').addClass("badge badge-active");
					}

					$('#waitting_result').text(e.target.options.waitting_result);
					$('#waitting_result').removeClass("badge badge-active");
					if (e.target.options.waitting_result > 0) {
						$('#waitting_result').addClass("badge badge-active");
					}

					$('#comfirmed').text(e.target.options.comfirmed);
					$('#comfirmed').removeClass("badge");
					if (e.target.options.comfirmed > 0) {
						$('#comfirmed').addClass("badge");
					}

					$('#death').text(e.target.options.death);
					$('#death').removeClass("badge");
					if (e.target.options.death > 0) {
						$('#death').addClass("badge");
					}

					$('#newdeath').text(e.target.options.newdeath);
					$('#newdeath').removeClass("badge");
					if (e.target.options.newdeath > 0) {
						$('#newdeath').addClass("badge");
					}

					$('#recovered').text(e.target.options.recovered);
					$('#recovered').removeClass("badge");
					if (e.target.options.recovered > 0) {
						$('#recovered').addClass("badge");
					}

					$('#admit').text(e.target.options.admit);
					$('#admit').removeClass("badge");
					if (e.target.options.admit > 0) {
						$('#admit').addClass("badge");
					}

					$('#coma').text(e.target.options.coma);
					$('#coma').removeClass("badge");
					if (e.target.options.coma > 0) {
						$('#coma').addClass("badge");
					}
			...
			<div class="countcorona">
				...
				<div class="countcorona-area">
					<h4>กลุ่มเสี่ยงจำแนกรายอำเภอ</h4>
					<div>
						...
						<table id="total-summary" class="table-bordered stripe row-border order-column">
							<thead>
								<tr>
									...
									<th colspan="4" class="text-center">อื่นๆ</th>
									<th></th>
							...
							<tbody>
								...
								foreach ($district as $value) {
									...
									$bar_data .= "['" . $value['ampurname'] . "', " . $value['risk_korea_worker'] . ", " . $value['risk_cambodia_border'] . ", " . $value['q_close_to_case'] . ", " . $value['risk_from_bangkok'] . ", " . $value['q_from_risk_country'] . ", " .
									...
									$bar_data2 .= "['" . $value['ampurname'] . "', " . ($value['detected'] - $value['newcase_ended']) . ", " . $value['newcase_ended'] . ",''],";
								?>


									<tr>

										<td><a href="<?= Url::to(['district', 'ampurcodefull' => $value['ampurcodefull'] ? $value['ampurcodefull'] : '0']) ?>"><?= $value['ampurname'] ?></a></td>
										...
										<td><?= number_format(($value['newcase'] - $value['risk_korea_worker_new'] - $value['risk_cambodia_border_new'] - $value['q_close_to_case_new'] - $value['risk_from_bangkok_new'] - $value['q_from_risk_country_new']) )?></td>
										...
										<td><?= number_format($value['newcase_ended'] - $value['risk_korea_worker_ended']- $value['risk_cambodia_border_ended'] - $value['q_close_to_case_ended'] - $value['risk_from_bangkok_ended'] - $value['q_from_risk_country_ended']) ?></td>
										<td><?= number_format(($value['detected'] - ($value['risk_korea_worker'] + $value['risk_cambodia_border'] + $value['q_close_to_case'] + $value['risk_from_bangkok'] + $value['q_from_risk_country'])) - ($value['newcase_ended'] - $value['risk_korea_worker_ended']- $value['risk_cambodia_border_ended'] - $value['q_close_to_case_ended'] - $value['risk_from_bangkok_ended'] - $value['q_from_risk_country_ended'])) ?></td>
 ...
 ", View::POS_END, 'my-options');
							...
							<tfoot>
								<tr>
									...
									<th><?= number_format($total_newcase - $total_risk_korea_worker_new - $total_risk_cambodia_border_new - $total_q_close_to_case_new - $total_risk_from_bangkok_new - $total_q_from_risk_country_new) ?></th>
									<th><?= number_format($total_other) ?></th>
									<th><?= number_format($total_newcase_ended - $total_risk_korea_worker_ended - $total_risk_cambodia_border_ended - $total_q_close_to_case_ended - $total_risk_from_bangkok_ended - $total_q_from_risk_country_ended) ?></th>
									<th><?= number_format($total_other - ($total_newcase_ended - $total_risk_korea_worker_ended - $total_risk_cambodia_border_ended - $total_q_close_to_case_ended - $total_risk_from_bangkok_ended - $total_q_from_risk_country_ended) ) ?></th>
